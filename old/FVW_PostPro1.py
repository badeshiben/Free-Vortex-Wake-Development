...
def run_study(WS, name, values):
    ...
    plots of % diff between results at current and finest resolution
    """
    work_dir = 'BAR_02_discretization_inputs/' + study['param'] + '/' # Output folder
    for wsp in WS:
        i = WS.index(wsp)
        outFiles=[]
        for val in values:
        ...
        dfAvg.insert(0,study['param'], values)
        # --- Save to csv since step above can be expensive
        dfAvg.to_csv('Results_ws{:04.1f}_'.format(wsp) + name + '.csv', sep='\t', index=False)
        #print(dfAvg)
    resolution_raw(study['param'], WS, 2)
    resolution_pDiff(study['param'], WS, 2)
    spanwise_values(study['param'], WS, 2)
